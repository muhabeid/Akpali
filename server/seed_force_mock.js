...
db.serialize(async () => {
  console.log('Force seeding mock clients, tenders, LPOs, and company experience...');

  // Ensure clients
  db.run(`DELETE FROM clients`);
  db.run(`INSERT INTO clients (id, name, registration_num, tax_pin, contact_name, email, phone, address) VALUES
    ('CLI-101', 'UNHCR Sub-Office Turkana', 'UNHCR-KE-891', 'P051892014Z', 'Sarah Jenkins (Procurement)', 'turkana.procurement@unhcr.org', '+254 733 111 222', 'Kakuma Camp, Turkana'),
    ('CLI-102', 'County Government of Kiambu', 'CGK/PROC/2022', 'P051284910Y', 'Eng. Peter Kamau (Director)', 'infrastructure@kiambu.go.ke', '+254 722 333 444', 'County HQ, Kiambu Town'),
    ('CLI-103', 'KCB Group Headquarters', 'C.12948', 'P000621482X', 'David Ochieng (Facilities)', 'dochieng@kcbgroup.com', '+254 20 327 0000', 'Kencom House, Nairobi')
  `);

  // Ensure tenders
  db.run(`DELETE FROM tenders`);
  db.run(`INSERT INTO tenders (id, name, client_id, client_name, client_reference, category, contract_value, status, progress) VALUES
